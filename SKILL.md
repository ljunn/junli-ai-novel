...
需要方法论文档时，不要平扫 `references/`；先从 `references/wiki.md` 进，再按 hub 渐进式下钻。
...
- 如果是局部问题，优先从 `references/quality/micro-revision-ops.md` 中选择单一微操作
- 如果问题在主线或章节层，先回上层重查

参考：
- `references/wiki.md`
- `references/hubs/revision-quality.md`
...
按任务下钻：
- 立项 / 规划：`references/hubs/planning.md`
- 章节创作 / 续写：`references/hubs/chapter-execution.md`
- 返修 / 质检 / 风格：`references/hubs/revision-quality.md`
- 长篇治理：`references/hubs/governance.md`
- 营销 / 平台输出：`references/hubs/platform.md`

特殊规则：
- `references/chapter/writing-quickref.md` 会由 `plan` / `compose` 自动注入 intent 文件，无需手动读取
- `references/tooling/tooling-adoption-roadmap.md` 保持为“方法沉淀”文档，不作为默认创作入口
...
- 出现“男主膈应”“女主太憋屈”“人设塌了”时，优先读 `references/quality/style-guardrails.md`

## 外部材料接入

如果用户提供方法论、访谈、经验材料：

1. 先判断它归 `planning / chapter-execution / revision-quality / governance / platform` 哪个 hub
2. 先更新对应 hub 的入口说明，再决定是否补到原子 reference
3. 长篇治理相关优先并入 `references/governance/longform-governance.md`
4. 大纲修整相关优先并入 `references/planning/outline-refinement.md`
5. 分类词库、用辞范例、描写辞典类材料优先并入 `references/chapter/descriptive-taxonomy.md`
6. 局部返修相关优先并入 `references/quality/micro-revision-ops.md`
7. 工具与工作台相关优先并入 `references/tooling/tooling-adoption-roadmap.md`
8. 只吸收可操作、可复用、可验证的内容
9. 只适用于单项目的经验，写进项目文档，不污染通用规则
