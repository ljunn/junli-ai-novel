...
如果要读方法论文档，不要直接平扫 `references/`；先看 `references/wiki.md`，再按任务类型进入对应 hub。
...
- **本章写作约束**：`rules/novel-lint/*.yaml`、`references/quality/rule-linting.md`
- **一致性和质检**：`references/quality/consistency.md`、`references/quality/quality-checklist.md`
...
如需查同类开头、对白、章末停点或题材案例，直接走外部知识库 / MCP / 搜索工具；`plan` 生成的 intent 会给出建议查询词。
...
- `references/hubs/`：按任务分流的渐进式入口页
- `references/`：原子写作方法论
- `rules/novel-lint/`：规则化文本巡检
- `审阅意见/`：`review` 自动生成的章节审阅报告
- `references/platform/platform-output-gate.md`：平台输出门禁说明
